...
/**
 * Servlet implementation class Notice_Submitter
 */
@WebServlet("/Notice_Submitter")
public class Notice_Submitter extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		 ...
		 ps = con.prepareStatement("INSERT INTO notice_board (class_id,notice) VALUES(?,?)");
			 ps.setString(2, request.getParameter("notice"));
				 ...
				 ps=con.prepareStatement("SELECT LAST_INSERT_ID()"); 
					ResultSet rs=ps.executeQuery();
						if(rs.next()){
							...
							Notification notification=new Notification("NOTICE",request.getParameter("notice"),request.getParameter("ClassId"),(String) request.getSession(false).getAttribute("USERID"),user.getFullname(),user.getProfilepic(),content_id+"");
