...
public class AskQuestion extends HttpServlet {
	...
	if( request.getSession(false)!=null&&request.getParameter("ClassId")!=null){
		...
		DbConnection dbc=new DbConnection(); 
		  ...
		  int i;
		int flag1=1;
		if(request.getParameter("question")==null ||request.getParameter("question")==""||request.getParameter("question").isEmpty()){
...
if(flag1==1 &&check.get(4).equals("true")||check.get(0).equals("true")){
try {
     ...
     userdetails.connClose();
	  ...
	  ps = con.prepareStatement("INSERT INTO questioninwritten (topic,qes_content,qes_raiser,class_id,ques_raiser_name) VALUES(?,?,?,?,?)"); 
	 ps.setString(1, request.getParameter("topic"));
	
	 ps.setString(2,"<div>"+request.getParameter("question")+"</div>");
	 ...
	 ps.setString(5, user.getFullname());
	  ...
	  if(i==1){
			 ...
			 Notification notification=new Notification("Question","<div>"+request.getParameter("question")+"</div>",request.getParameter("ClassId"),(String) request.getSession(false).getAttribute("USERID"),user.getFullname(),user.getProfilepic(),content_id+"");
