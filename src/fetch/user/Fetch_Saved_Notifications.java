...
/**
 * Servlet implementation class Fetch_Saved_Notifications
 */
@WebServlet("/Fetch_Saved_Notifications")
public class Fetch_Saved_Notifications extends HttpServlet {
		...
		if( request.getSession(false).getAttribute("ISLOGIN")!=null &&request.getSession(false).getAttribute("ISLOGIN").equals("true")&& request.getSession(false).getAttribute("USERID")!=null){
		 ...
		 Fetch_saved_items fetch=new  Fetch_saved_items();
			...
			response.setContentType("application/json"); 
			   ...
			   response.getWriter().write(fetch.Fetch_saved_notifications((String) request.getSession(false).getAttribute("USERID")));
	}
		else{
